...
export default {
	...
	computed: {
		sizes() {
			if (this.paramInfo.sizes.length === 1) return this.paramInfo.sizes[0];
			let spliceArr = [];
			...
			for (let i = 0; i < spliceArr.length; i++) {
				this.paramInfo.sizes[0][i].push(spliceArr[i]);
			}
			return this.paramInfo.sizes[0];
		}
	}
};
</script>
...
.rule table {
	...
	border-bottom: 0.17rem solid #cccccc;
}
.size table tr,
.rule table tr {
